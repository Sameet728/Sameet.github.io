...
if(formData.number.length == 10 || formData.number.length == 0){
  ...
  return (
   ...
   <div className='flex flex-col items-center gap-2'>
          <h2 id='animation' className='text-1/2xl text-gray-500'>Get in Touch</h2>
...
<div className='flex flex-col-reverse sm:flex-row '>
  ...
  <h2 id='animation' className='text-3xl font-semibold text-gray-500  '><span className='text-gray-500'>📞</span> Get in Touch</h2>
  ...
  <h2 id='animation' className=' font-semibold text-gray-600 text-2xl sm:pl-10 sm:text-3xl'>Love to hear from you 👋</h2>
  
  <div className='mt-10'>
    <div className='flex flex-col sm:flex-row   gap-8 '>
    <div id='animation'>
      <label htmlFor="name" className='font-semibold text-gray-900'>Your Name</label>
     <Input required onChange={handleInputChange} value={formData.name} name="name" className='w-[105%]  sm:w-[440px] text-1/2xl font-bold mt-1 bg-gray-50' placeholder="Your full name, please" type="text"/>
    </div>
    <div id='animation'>
    <label htmlFor="email" className='font-semibold text-gray-900 '>Email</label>
     <Input onChange={handleInputChange}  value={formData.email} name="email" className='w-[105%]  sm:w-[440px] text-1/2xl font-bold mt-1 bg-gray-50' placeholder="Drop your email address"  type="email"/>
    </div>
    </div>
    <div  className='flex flex-col sm:flex-row    gap-8 mt-8'>
    <div id='animation'>
      <label htmlFor="name" className='font-semibold text-gray-900'>Phone Number (optional)</label>
     <Input onChange={handleInputChange}  value={formData.number} name="number" className='w-[105%]  sm:w-[440px] text-1/2xl font-bold mt-1 bg-gray-50' placeholder="Leave your number, if you'd like" type="number"/>
    </div>
    <div id='animation'> 
    <label htmlFor="name" className='font-semibold text-gray-900'>Select your preferred contact method  </label>
    <Select onValueChange={handleSelectChange}>
  <SelectTrigger className="w-[105%]  sm:w-[440px] text-1/2xl font-semibold  mt-1 bg-gray-50 ">
  ...
  <SelectContent>
    ...
    <Textarea onChange={handleInputChange}  value={formData.message} name="message" className=' text-1/2xl font-bold mt-1 bg-gray-50 w-[105%]  sm:w-[920px]' placeholder="Feel free to share the details here"></Textarea>
     
    </div>

  </div>
  <div id='animation' className='flex justify-center sm:justify-end items-center sm:pr-36 pt-10'><Button onClick={sendHandler}>{loading ? <><FaSpinner  className="animate-spin mr-2 text-white" />
